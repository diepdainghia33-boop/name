          ...
          >
            <div className="mt-4 rounded-[28px] border border-white/8 bg-white/[0.02] p-4 shadow-[0_24px_70px_rgba(0,0,0,0.35)] backdrop-blur-xl">
              <div className="flex items-center justify-between gap-4 pb-4">
                <div>
                  <p className="text-[0.625rem] font-medium uppercase tracking-[0.32em] text-foreground/40">
                    Navigation
                  </p>
              ...
              <div className="grid gap-2">
                {navItems.map((item) => (
                  <button
                    ...
                    type="button"
                    onClick={() => setMobileOpen(false)}
                    className="flex items-center justify-between rounded-2xl border border-border/70 bg-background/50 px-4 py-3 text-left text-[0.9375rem] text-foreground/90 transition-colors hover:border-white/15 hover:bg-white/[0.05] active:scale-[0.99]"
                  >
                    <span>{item.label}</span>
