...
class ChatController extends Controller
{
    ...
    public function duplicateConversation($id)
    {
        $original = Conversation::where('id', $id)
            ->where('user_id', Auth::id())
            ->firstOrFail();
        ...
        // Copy messages
        $messages = Message::where('conversation_id', $original->id)
            ->orderBy('created_at', 'asc')
            ->get();

        foreach ($messages as $msg) {
            $newMsg = $msg->replicate();
            $newMsg->conversation_id = $newConv->id;
            
            // Handle associated Bill if exists
            if ($msg->bill_id) {
                $oldBill = Bill::find($msg->bill_id);
                if ($oldBill) {
        ...
        $newConv->loadCount('messages');
        $newConv->loadSum('messages', 'tokens');
        $newConv->load('latestMessage');

        return response()->json([
            'message' => 'Conversation duplicated successfully',
            'conversation' => $this->formatConversation($newConv),
        ]);
    }
}
