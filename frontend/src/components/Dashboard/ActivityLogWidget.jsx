...
export default function ActivityLogWidget({ logs }) {
    ...
    return (
        <div className="flex h-80 flex-col overflow-hidden rounded-[28px] border border-border/70 bg-surface p-6 shadow-[0_18px_40px_rgba(0,0,0,0.24)]">
                ...
                {logs.length === 0 ? (
                    ...
                    logs.map((log, i) => (
                        <div key={log.id || i} className="relative rounded-[20px] border border-border/70 bg-background px-4 py-3">
                            ...
                            <p className="text-[0.6875rem] font-bold text-text">{getLogLabel(log)}</p>
                            <p className="mt-1 text-[0.625rem] leading-relaxed text-text-muted">{log.message}</p>
            ...
            <button className="mt-6 rounded-2xl border border-border/70 bg-background px-4 py-2 text-[0.625rem] font-black uppercase tracking-[0.24em] text-muted transition-colors hover:border-accent/40 hover:text-text">
                View full archive
            </button>
        </div>
    );
